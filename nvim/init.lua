...
local dontInstall = {
  -- installed externally due to its plugins: https://github.com/mason-org/mason.nvim/issues/695
  "stylelint",
...
local function toolsToAutoinstall(myLinters, myFormatters, myDebuggers, ignoreTools)
  ...
  -- only unique tools
  table.sort(tools)
  local utools = vim.fn.uniq(tools)

  -- remove exceptions not to install
  utools = vim.tbl_filter(function(tool)
...
require("lazy").setup({
  {
    ...
    opts = {
      ...
      scroll = { enabled = false }, -- Don't like the feeling.
